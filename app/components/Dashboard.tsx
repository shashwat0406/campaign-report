...
import type { OneXtelData } from "@/lib/sheet";
import { useTheme } from "@/lib/theme";
import { refreshData } from "@/app/actions";
import { TrendChart, LeadsBar, DeliveryDonut, Sparkline } from "./charts";

const nf = (n: number) => n.toLocaleString("en-IN");
const pct = (n: number, d = 1) => `${n.toFixed(d)}%`;

// Renders the fetch time only after mount to avoid SSR/client timezone mismatch.
...
export default function Dashboard({ data }: { data: OneXtelData }) {
  ...
  const { days, totals: t } = data;
  const active = days.filter((d) => d.sent != null);

  const spark = (k: "sent" | "delivered" | "read" | "leads" | "leadPct") =>
  ...
  const kpis = [
    ...
    { label: "Avg Cost / Lead", val: "₹—", meta: "awaiting spend data", spark: spark("leadPct"), feat: false },
  ];
  ...
  const failed = Math.max(t.sent - t.delivered, 0);
      ...
      <div className="wrap">
        ...
        <header className="top">
          <div className="brand">
            ...
            <div>
              ...
              <h1>OneXtel Campaign Report</h1>
              <p className="sub">Gold-loan lead acquisition · daily campaign tracking</p>
            </div>
          </div>
          <div className="chips">
            ...
            <span className={`chip ${data.source}`}>
              <span className="dot" />
              {data.source === "live" ? "Live · sheet" : "Snapshot"}
            </span>
            <button
              className="chip button"
              onClick={refresh}
              disabled={pending}
              aria-label="Refresh data from sheet"
            >
        ...
        <section className="grid">
          <div className="card">
            <div className="card-head">
              ...
              <span className="note">12 Aug had no campaigns</span>
            </div>
            ...
            <div className="donut-legend">
              ...
              {[
                ...
                <div className="dl" key={s.n}>
                  ...
                  <span className="pc">{pct((s.v / t.sent) * 100, 0)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="card table-card">
          ...
          <div className="tscroll">
            <table>
              <thead>
                <tr>
                  <th>Date</th><th>Camps</th><th>Sent</th><th>Delivered</th><th>Del %</th>
                  <th>Read</th><th>Leads</th><th>Lead %</th><th>Unqual %</th>
                </tr>
              </thead>
              <tbody>
                {days.map((d) =>
                  d.sent == null ? (
                    <tr className="muted-row" key={d.date}>
                      <td>{d.date}</td>
                      <td>{d.campaigns ?? 0}</td>
                      <td colSpan={7}>No campaigns sent</td>
                    </tr>
                  ) : (
                    <tr key={d.date}>
                      <td>{d.date}</td>
                      ...
                      <td>
                        {d.deliveryPct != null ? (
                          <span className={`tag ${delTag(d.deliveryPct)}`}>{pct(d.deliveryPct)}</span>
                        ) : "—"}
                      </td>
                      <td>{nf(d.read ?? 0)}</td>
                      <td>{d.leads ?? "—"}</td>
        ...
        <div className="foot">
          <span>
            ...
            {data.source === "live" ? "live via gviz" : "bundled snapshot (sheet not public yet)"} ·{" "}
            <UpdatedAt iso={data.fetchedAt} />
          </span>
          <span>Delivery avg {pct(t.deliveryPct)} · Lead rate {pct(t.leadPct, 2)} of sent</span>
        </div>
      </div>
    </div>
  );
